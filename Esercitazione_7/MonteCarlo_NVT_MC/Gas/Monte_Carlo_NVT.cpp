...
//update of the histogram of g(r)
        if (dr<box/2.0)    walker[igofr + int( dr/bin_size) ] += 2;
     if(dr < rcut)
     {
...
//    ofstream WriteWalker;
//    WriteWalker.open("output_walker_epot_press.dat",ios::app);
//    WriteWalker << walker[iv]/npart + vtail << '\t' << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
//    WriteWalker.close();
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Gave.open("Risultati/output_gave.dat",ios::app);
...
//g(r)
    ...
    for (int i=igofr; i<igofr+nbins; ++i){
        r=i*bin_size;
        gofr_gasid= rho*npart*((4./3.)*M_PI*((r+bin_size)*(r+bin_size)*(r+bin_size)-r*r*r));
        

        stima_gofr = blk_av[i]/(blk_norm*gofr_gasid);
        glob_av[i]+= stima_gofr;
        glob_av2[i] += stima_gofr*stima_gofr;
        err_gofr=Error(glob_av[i],glob_av2[i],iblk);
        
        Gofr << iblk << '\t' << r << '\t' << stima_gofr << endl;;
    }
    
    
    for(int i=igofr; i<igofr+nbins; ++i){
        Gave << iblk << '\t' << glob_av[i]/(double)iblk << '\t' << err_gofr << endl;
     }
